...
type Contest = Omit<CrawledContest, "source">;
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        mapRef.current = new mapsWindow.google.maps.Map(container.current, { center: centerRef.current ?? { lat: 37.5665, lng: 126.978 }, zoom: 12, disableDefaultUI: true, zoomControl: true, streetViewControl: false });
  ...
  useEffect(() => {
    ...
    contests.forEach((contest, index) => {
      ...
      geocoder.geocode({ address: contest.location, region: "KR" }, (entries: any[], status: string) => {
        const point = status === "OK" && entries[0] ? { lat: entries[0].geometry.location.lat(), lng: entries[0].geometry.location.lng() } : null;
...
export default function Home() {
  ...
  const [query, setQuery] = useState(""); const [tag, setTag] = useState("전체"); const [radiusKm, setRadiusKm] = useState(10);
  ...
  const offline = useMemo(() => offlineCandidates.filter((contest) => { const point = coordinates[contest.id]; return !!point && (!center || distanceKm(center, point) <= radiusKm); }), [center, coordinates, offlineCandidates, radiusKm]);
